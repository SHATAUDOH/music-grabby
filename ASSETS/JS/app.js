...
const notFound = document.getElementById('nothing');
...
//create a function for the button click event
...
async function makeRequest() {
    ...
    try {
        ...
        if (result.length === 0) {
            notFound.innerHTML = "cannot find the music";
        }
        else{
            //loop through
            result.forEach((single) => {
                ...
                const yearAvailable = single.track.track_year ? single.track.track_year : 'year not available';
